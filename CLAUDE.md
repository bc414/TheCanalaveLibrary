...
`.claude/workplan.md` (Position block) → `.claude/roadmap.md` (phases + open decisions) — then the
...
| `.claude/workplan-archive.md` | Older DONE work-unit entries, moved out of `workplan.md` to keep the live ledger navigable. Append-only; "workplan.md WU-X" citations resolve here for archived entries. | Archival sweeps (trigger: workplan.md > ~1,500 lines → move DONE entries older than ~2 weeks) |
...
| `.claude/skills/doc-audit/SKILL.md` | The fresh-eyes doc-audit method: three probe shapes (cold-session walk, restructure integrity check, untouched-tail probe) run as subagents after doc restructures and at phase gates. Catches the contradiction/tense/dead-identifier classes the mechanical gates cannot. | If a probe shape or exemption changes |
...
| `.claude/design/` (other files) | Standing cross-cutting analyses that back build passes: `access-gating-first-principles.md` (the authoritative Feature-66 viewer-permission model; `access-gating-audit.md` is its surface inventory — first-principles wins on disagreement), the two L6 evidence reports (`L6-intent-ledger.md`, `L6-reconciliation-matrix.md` — moved from `audit/` 2026-07-27; the matrix's live-`pg_indexes` reconciliation is still PENDING), `db-schema-first-principles-audit.md` (2026-08-02 whole-schema logical audit: spec §4 vs. shipped DDL vs. first principles; pre-lock-in defect list + decision batch), and `service-layer-first-principles-audit.md` (2026-08-03 whole-L2 audit, companion to the schema audit: spec ∩ shipped services ∩ first principles; systemic defect families + decision batch + doctrine corrections). | The pass that consumes them |
| `.claude/roadmap.md` | Live phased master plan (platform-first → features → beta → launch) + "Decisions that need you" table (open items) and a "Resolved" list (closed items going forward, each pointing at the convention doc that now states the rule). Supersedes the retired `.claude/middle_plan_v2.md` (2026-07-27), which itself superseded `.claude/middle_plan.md` and `.claude/forward_plan.md` (all three historical references now; the full pre-2026-07-27 Resolved index lives in `middle_plan_v2.md`, not duplicated here). | Whoever resolves a decision or advances a phase |
