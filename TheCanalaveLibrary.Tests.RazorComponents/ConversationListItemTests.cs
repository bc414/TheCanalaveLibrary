...
/// renders when <c>UnreadCount &gt; 0</c> and is absent when zero; IsSelected changes the
/// highlight class.
///
/// The WU-MsgArchive-era "no archived chip" render tests were retired by WU-MsgReadPath
/// (2026-07-26): <see cref="ConversationSummaryDto"/> no longer carries an archived flag at all
/// (listings are scoped via <see cref="ConversationScope"/>), so a per-row archived marker is
/// now structurally impossible — the compiler enforces what those tests pinned. The
/// unread-badge-survives-archiving invariant is pinned where the state actually lives:
/// <c>MessagesPageTests</c> (page level) and the Integration sticky-archive test (service level).
///
/// <b>Tier:</b> RazorComponents (bUnit, no host or DB).
/// </summary>
public class ConversationListItemTests : BunitContext
{
    private static ConversationSummaryDto MakeConversation(int unreadCount = 0)
        => new(
            ConversationId: 1,
