...
| 13 | **`/discover` URL state round-tripping** *(added 2026-07-27 — the blocking question inside tracker item B11, promoted here so it lives in a decision ledger; full framing + the three options at `hidden-deferrals-tracker.md` B11)*: should `/discover` round-trip filter state through the URL at all? (1) yes, all axes, `TreeSearchPage`-style — shareable searches, back/forward work; (2) no URL state but add ship seeding anyway — closes the ships-die-on-navigation asymmetry cheaply; (3) leave as-is — only defensible if URL round-tripping is rejected permanently. | None — B11 names option 3 "deferral wearing a decision's clothes" absent an explicit ruling. | Shareable-URL product behavior + a privacy-perception call (tag ids visible in URLs). Gates only tracker B11; no phase gate. |
