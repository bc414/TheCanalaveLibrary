...
- **`Email/`** (Server only) — `IMailTransport`/`OutgoingMail` + the SMTP and NoOp implementations + `EmailOptions`: the app's single outbound-mail seam, consumed by Identity's transactional mail (`SmtpEmailSender`) and by notification fan-out (`NotificationEmailFlusher`). Appeared at WU-NotifEmail (2026-07-31) when the second consumer arrived; before that it was Identity's private concern. Server-only, like `ISpriteAssetProbe` — nothing in Core, Client, or SharedUI sends mail. Boundary: this cluster owns *how a message is transmitted*; each consuming feature owns *what the message says* (`Identity/EmailBodies.cs`, `Notifications/NotificationEmailBodies.cs`).
