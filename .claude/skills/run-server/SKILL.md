...
discovery, OpenTelemetry traces in the dashboard, or the deferred N≥2 scale-out story for the
signal buffers — see `canalave-conventions/horizontal-scaling.md`; the `cache` container is
consumed by nothing at N=1) or when verifying the app
