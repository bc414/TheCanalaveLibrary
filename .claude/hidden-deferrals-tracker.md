...
- [ ] **A3 — Full story-completion tracking → spoiler gate is hardcoded open** `[scope-cut · med · beta]`
  - Grid: F7 L3-Logic=5; F26 (Spoiler Comments) all built cells=5.
  - Source: `audit/Chapters.md` WU26 note; `audit/Comments.md` Feature 26.
  - Context: "Full completion tracking is post-MVP." `CommentSection` is wired with `UserHasCompletedStory=false` **hardcoded**, so the spoiler completion-gate can never treat a viewer as having finished the story — the gate is effectively inert in one direction. Both features read Stage 5.
  - Next: build completion tracking, then replace the hardcoded `false` with the real signal.
