    ...
    ITagHierarchyReadService tagHierarchy) : IStoryReadService
{
    ...
    private async Task<TagExpansionMap> ResolveExpansionAsync(StoryFilterDto filter)
    {
        ...
        return StoryFilterPredicates.NamesTagIds(filter)
            ? await tagHierarchy.GetExpansionMapAsync()
            : TagExpansionMap.Empty;
    }

    /// <summary>
    /// Thin instance-side alias for <see cref="StoryFilterPredicates.ApplyFilters"/> — the shared
    /// predicate moved out to <c>StoryFilterPredicates</c> (WU-ExploreFilterAxes, 2026-07-31) so
    /// Manual Tree Search's Explore pane narrows by identical semantics. Kept as a local name
    /// because three call sites read better without the type prefix; the behavior, purity
    /// invariant, and roll-up rules are documented on the shared class.
    /// </summary>
    private static IQueryable<Story> ApplyFilters(
        ...
        StoryFilterPredicates.ApplyFilters(query, filter, expansion, viewerId, hasFts);
